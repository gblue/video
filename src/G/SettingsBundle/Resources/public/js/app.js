...
app.controller('TranslationCtrl', ['$scope', '$http', '$timeout', 'ngTableParams', 'sharedMessage', function($scope, $http, $timeout, ngTableParams, sharedMessage) {
    ...
    $scope.columns = [
        ...
        { title: translationCfg.label.label, index: 'label', edit: translationCfg.editable.label, filter: {'label': 'text'}, sortable: true, visible: translationCfg.visible.label },
        { title: translationCfg.label.name, index: 'name', edit: translationCfg.editable.name, filter: {'name': 'text'}, sortable: true, visible: translationCfg.visible.name },
        { title: translationCfg.label.value, index: 'value', edit: translationCfg.editable.value, filter: {'value': 'text'}, sortable: true, visible: translationCfg.visible.value },
        { title: translationCfg.label.visible, index: 'visible', edit: translationCfg.editable.visible, filter: {'visible': 'text'}, sortable: true, visible: translationCfg.visible.visible }
    ];
        ...
        getData: function($defer, params) {
            ...
            $http.get(translationCfg.url.list, {'params': parameters}).success(function (responseData) {
                $timeout(function() {
                    ...
                    $defer.resolve(responseData.settings);
                }, 100);
            });
        }
    };
        ...
        link: function ( $scope, element, attrs ) {
            ...
            $scope.save = function (event) {
                if (event.which == 27) { // ecsape key
                    ...
                    var url = translationCfg.url.update.replace('-id-', $scope.setting.id);

                    var parameters = [];
                    for (var name in $scope.setting) {
                        parameters.push(name+'='+encodeURIComponent($scope.setting[name]));
                    }
                    ...
                    $http({ 'url': url, 'data': parameters.join('&'), method: 'PUT', headers: {'Content-Type': 'application/x-www-form-urlencoded'} })
                        .success(function (data, status, headers, config) {
                            ...
                            sharedMessage.set('text-danger', 'remove-circle', translationCfg.label.errorMsg.replace('%id%', $scope.setting.id));
                        })
                    ;
                }
            };
